...
Use the same signed-in account and Thorium browser, fresh personalized Temporary Chats, German UI and prompt, 400–600 requested words, and no search, plugins or external sources. Explicitly verify the model and personalization before every submission. Remove any optional selected plugin from the new empty composer without changing global settings. Preserve user memory/custom instructions. Record visible context before and after; visible sameness does not prove identical hidden retrieval.
