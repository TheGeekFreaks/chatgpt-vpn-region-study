...
Regular trial chats enter the account history and may affect later memory/retrieval. At the user's explicit instruction, after each completed response first preserve the exact response and its measurement data privately, then delete only that newly created test conversation and verify its removal before starting the next trial. Never delete a historic user conversation or an unarchived trial. Retain private conversation identifiers and deletion receipts outside the public repository. Deleting a conversation does not guarantee removal of any separately updated memory; randomized order and complete blocks do not eliminate this residual carryover. Record order and visible context changes and report this limitation. Do not claim that the prompt or regular mode guarantees access to every historic chat. Do not silently substitute temporary chats or disable history to improve experimental control.
